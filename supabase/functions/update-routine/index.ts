...
Deno.serve(async (req) => {
  try {
    ...
    if (routine_name) {
      const { error: updateErr } = await supabase.from("routines").update({
          ...
          if (needsCopy) {
            ...
            if (currentWorkoutId) {
              const { data: sourceData } = await supabase
                .from("workouts")
                .select(
                  `*, workout_exercises(*, exercises(*), exercise_sets(*))`,
                )
                .eq("workout_id", currentWorkoutId)
                .maybeSingle();

              if (sourceData) {
                const sortedExercises = (sourceData.workout_exercises || [])
                ...
                workoutToCopyDetails = {
                  ...
                  exercises: sortedExercises.map((we: any) => ({
                    ...
                    setTargets: (we.exercise_sets || []).sort((
                      a: any,
                      b: any,
                    ) => a.set_number - b.set_number).map((s: any) =>
                      s.details
                    ),
              ...
              for (let i = 0; i < exs.length; i++) {
                ...
                if (!exerciseId) {
                  const { data: existingEx } = await supabase.from("exercises")
                    .select("exercise_id").eq("exercise_name", ex.name || "")
